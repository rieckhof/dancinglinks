...
std::vector<size_t> create_test_results(std::shared_ptr<DancingLinks::ColumnObj> iterator_down,
...
void check_all_up(std::shared_ptr<DancingLinks::ColumnObj>const& iterator, std::vector<size_t>const& expected_values)
{
    std::vector<size_t> result;
    result.push_back(iterator->index);
...
void check_all_directions(DancingLinks const& dl, std::vector<std::vector<size_t>>& all_expected_values){
